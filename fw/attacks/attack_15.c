/*
 * fw/attacks/attack_15.c - Attack function with ID 15
 *
 ...
 * followed by a 127-byte MAC Data packet,
...
#error "Unknown transceiver"
	...
	/* Wait for the transmission of the spoofed MAC Data packet */
	_delay_us(560);

	/* Spoof a MAC Data packet */
	spi_begin();
	spi_send(AT86RF230_BUF_WRITE);
	spi_send(127);
	spi_send(0x79);
